...
def strip_cargo_toml():
    ...
    with open(path) as f:
        src = f.read()
    # Remove not(android) build-deps (cxx-qt-build)
    src = re.sub(
        r'\[target\.\'cfg\(not\(target_os = "android"\)\)\'\.build-dependencies\].*?(?=\n\[|\Z)',
        '', src, flags=re.DOTALL
    )
    # Remove linux deps (cxx/cxx-qt-lib)
    src = re.sub(
        r'\[target\.\'cfg\(target_os = "linux"\)\'\.dependencies\].*?(?=\n\[|\Z)',
        '', src, flags=re.DOTALL
    )
    # Remove tauri-plugin-system-info (doesn't support Android via starship-battery)
...
def cleanup():
    for fn in ['app/src-tauri/src/qt_app.cpp','app/src-tauri/src/qt_app.h','app/src-tauri/src/ipc_bridge.rs']:
